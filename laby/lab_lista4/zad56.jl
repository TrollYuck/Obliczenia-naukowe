...
for n in [5, 10, 15]
    ...
    title!("Interpolacja exp(x) dla n=$n węzłów równoodległych")
    xlabel!("x")
...
for n in [5, 10, 15]
    ...
    title!("Interpolacja abs(x) dla n=$n węzłów równoodległych")
    xlabel!("x")
    ylabel!("Nn,f(x) oraz f(x)")
    ...
    title!("Interpolacja 1/(1+x^2) dla n=$n węzłów równoodległych")
    xlabel!("x")
    ylabel!("Nn,f(x) oraz f(x)")
    ...
    title!("Interpolacja abs(x) dla n=$n Czebyszewa")
    xlabel!("x")
    ylabel!("Nn,f(x) oraz f(x)")
    ...
    title!("Interpolacja 1/(1+x^2) dla n=$n Czebyszewa")
    xlabel!("x")
    ylabel!("Nn,f(x) oraz f(x)")
